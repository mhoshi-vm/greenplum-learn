...
locals {
  prefix= "${var.prefix}${var.gp_release_version}"
  gp_virtual_internal_ip_cidr = "192.168.${random_integer.internal_subnet.result}.0/24"
  ...
  userfile_vars = {
    ...
    pxf_product_slug = var.pxf_product_slug
    pxf_release_version = var.pxf_release_version
    gptext_product_slug = var.gptext_product_slug
    gptext_release_version = var.gptext_release_version
  }
}

resource "random_integer" "internal_subnet" {
  min = 0
  max = 255
  keepers = {
    # Generate a new integer each time we switch to a new listener ARN
    gpv-prefix=local.prefix
  }
}
...
resource "vsphere_compute_cluster_vm_anti_affinity_rule" "coordinator_vm_anti_affinity_rule" {
    ...
    name                = format("%s-coordinator-vm-anti-affinity-rule", local.prefix)
...
resource "vsphere_compute_cluster_vm_anti_affinity_rule" "segment_vm_anti_affinity_rule" {
    ...
    mandatory           = true
    compute_cluster_id  = data.vsphere_compute_cluster.compute_cluster.id
    name                = format("%s-segment-vm-anti-affinity-rule-sdw%0.3d-sdw%0.3d", local.prefix, count.index*2+1, count.index*2+2)
    virtual_machine_ids = [
        vsphere_virtual_machine.segment_hosts.*.id[count.index*2],
        vsphere_virtual_machine.segment_hosts.*.id[count.index*2+1],
    ]
}
