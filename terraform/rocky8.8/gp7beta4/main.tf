...
variable "gp_virtual_external_ipv4_addresses" {
  ...
  default = ["192.168.100.30" , "192.168.100.100", "192.168.100.150"]
}
...
locals {
  ...
  segment_gp_virtual_internal_ipv4_offset = 130
  segment_gp_virtual_etl_bar_ipv4_offset = 130
  gp_virtual_internal_ipv4_netmask = parseint(regex("/(\\d+)$", local.gp_virtual_internal_ip_cidr)[0], 10)
  master_internal_ip = cidrhost(local.gp_virtual_internal_ip_cidr, (pow(2,(32 - local.gp_virtual_internal_ipv4_netmask))-1)-5)
  standby_internal_ip = cidrhost(local.gp_virtual_internal_ip_cidr, (pow(2,(32 - local.gp_virtual_internal_ipv4_netmask))-1)-4)
